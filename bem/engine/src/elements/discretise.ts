...
//
// For now: a uniform split into N elements per line. Each element has 3
...
import { STANDARD_NODES, type LocalNodes } from "./shapeFunctions.js";
...
export function discretiseLines(
  ...
  for (const line of model.lines) {
    ...
    for (let i = 0; i < n; i++) {
      const tStart = i / n;
      const tEnd = (i + 1) / n;
      const start = pointAtT(line, tStart, points);
      const end = pointAtT(line, tEnd, points);
      ...
      for (const eta of nodes) {
        // η ∈ [-1, +1] → local fraction ∈ [0, 1] within this element.
        const local = (eta + 1) / 2;
        const t = tStart + local * (tEnd - tStart);
        const p = pointAtT(line, t, points);
        if (!p) {
          abort = true;
          break;
        }
      ...
      if (abort || nodePts.length !== 3) continue;
